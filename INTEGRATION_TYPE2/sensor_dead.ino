/*
  ...
  //gps　　gps_getの返り値を利用する。。？

  //funcs -> get_ac ないxbprintfを消去してみる？？
*/
...
void sd_dora() {
  xbee_uart(dev, "judging life of SD...\r");
  int cnt_dead, sno = 0;
  File samplefile = SD.open(SD_FLAG);
  boolean dead;
  while (sno < 50) {
    File samplefile = SD.open(SD_FLAG);
    dead = (!digitalRead(SD_INSERT) || (samplefile == false)); //ファイルを開けない・SD挿入YN回路から非挿入
    if ( dead ) {
      cnt_dead += 1;
    }
    samplefile.close();
...
void xbee_dora() {  // いつも接続を施工し続けるべきだからむしろ定期的にネットワーク参加を試行するプログラムにするべき。。？
