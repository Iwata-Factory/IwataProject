	...
	len=0;
		...
		if( len ){
			...
			if( sci_write_check() > 0 ){	// XBEE_WIFIなのでifdef LITEは不要
				#ifdef XBEE_WIFI_DEBUG
					Serial1.print("TX udp size=");
					Serial1.print(len);
					Serial1.print(", ");
					for(i=0;i<len;i++){
						Serial1.print(data_api[i],HEX);
						Serial1.print(" ");
					}
					Serial1.println("");
				#endif
				...
				if( sci_write( data_api, len ) == 0 ){
					...
					#endif
					#ifdef XBEE_WIFI_DEBUG
						Serial1.println("ERR:TX Write Error" );
					#endif
					#ifdef DEBUG
...
*/
	...
	unsigned int wait_add = 0;
		...
		#ifdef XBEE_WIFI_DEBUG
			for(i=0;i<(5+len);i++) Serial1.print(at[i]);
			Serial1.print('(');
			Serial1.print(PACKET_ID,HEX);
			Serial1.println(')');
		#endif
